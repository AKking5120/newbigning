      ...
      <Link href={`/product/${product.slug}`} className="block">
        ...
        <div className="relative overflow-hidden bg-zinc-900 aspect-[3/4]">
          ...
          <button
            ...
            className={`absolute top-3 right-3 w-8 h-8 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all ${
              isInWishlist
                ? "bg-crimson"
                : "bg-black/60 hover:bg-crimson"
            }`}
            aria-label={isInWishlist ? "Remove from wishlist" : "Add to wishlist"}
          >
            <Heart className={`w-4 h-4 ${isInWishlist ? "text-pearl fill-pearl" : "text-white"}`} />
          </button>
